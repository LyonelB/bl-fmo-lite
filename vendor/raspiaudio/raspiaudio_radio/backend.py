...
class RadioBackend:
    def __init__(self, config: RadioConfig) -> None:
        ...
        self._rds_thread_stop = False
        import threading as _th
        _th.Thread(target=self._rds_poll_thread, daemon=True, name='rds-poll').start()
    ...
    def _wait_fm_signal_locked(self, require_hd: bool) -> Optional[Dict[str, Any]]:
        ...
        if not require_hd:
            ...
            for _ in range(3):
                signal = self._merge_fmhd_status(radio.fm_rsq_status(attune=False), radio.hd_digrad_status())
    ...
    def _read_current_signal_locked(self) -> Dict[str, Any]:
        ...
        if station["mode"] == "dab":
            ...
            return signal
        if station["band"] == "fm":
            return self._merge_fmhd_status(radio.fm_rsq_status(attune=False), radio.hd_digrad_status())
        return self._merge_fmhd_status(radio.am_rsq_status(attune=True), radio.hd_digrad_status())

    def _rds_poll_thread(self):
        import time as _time
        _time.sleep(5)
        # Vider le FIFO des groupes stale au demarrage
        try:
            with self._lock:
                r = self._radio
            if r and hasattr(r, 'fm_rds_status'):
                r.fm_rds_status(intack=True, mtfifo=True)
                _time.sleep(1)
        except Exception:
            pass
        while not self._rds_thread_stop:
            try:
                with self._lock:
                    radio = self._radio
                    band  = (self._mode_info() or {}).get('band', '')
                    if radio and band == "fm" and hasattr(radio, "fm_rds_status"):
                        for _ in range(4):
                            group = radio.fm_rds_status(intack=True)
