...
class Si468xDabRadio:
    ...
    def _wait_cts(self, timeout: float = 1.0, allow_cmd_error: bool = False) -> None:
        ...
        while time.time() < deadline:
            try:
                # BL-FMO-LITE: CTS est dans resp[0], pas resp[1]
                if self.use_spi and self.spi is not None:
    ...
    def configure_fmhd_frontend(self) -> None:
        ...
        # Activer la reception RDS
        self.set_property(0x3C02, 0x0001)  # FM_RDS_CONFIG: enable RDS
        self.set_property(0x3C00, 0x0001)  # FM_RDS_INT_SOURCE: RDSRECV
    ...
    def fm_rds_status(self, intack=False, mtfifo=False):
        ...
        reply = self._read_reply(21)
        if len(reply) < 16:
            return {'sync': False, 'fifo_used': 0}
        return {
            'sync':      bool(reply[5] & 0x10),
            'fifo_used': reply[6],
            'block_a':   int.from_bytes([reply[8],  reply[9]],  'little'),
            'block_b':   int.from_bytes([reply[14], reply[15]], 'little'),
            'block_c':   int.from_bytes([reply[16], reply[17]], 'little'),
            'block_d':   int.from_bytes([reply[18], reply[19]], 'little'),
    ...
    def close(self) -> None:
        try:
            ...
            try:
                # BL-FMO-LITE patch: ne pas cleanup rst_pin
                # pour maintenir le chip actif apres fermeture SPI
